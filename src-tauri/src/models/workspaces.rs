    ...
    let updated = connection
        .execute(
            ...
            SET mode = 'worktree',
                ...
                worktree_path = ?3,
                branch = ?4,
                intended_target_branch = ?5,
                initialization_parent_branch = ?6,
                updated_at = ?7
...
fn workspace_record_from_row(row: &Row<'_>) -> rusqlite::Result<WorkspaceRecord> {
    Ok(WorkspaceRecord {
        ...
        display_order: row.get(35)?,
        repo_sidebar_order: row.get(36)?,
        created_at: row.get(37)?,
        updated_at: row.get(38)?,
        last_user_message_at: row.get(39)?,
        setup_completed_at: row.get(40)?,
