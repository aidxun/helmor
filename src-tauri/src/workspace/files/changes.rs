...
pub(super) fn query_workspace_target_by_id(
    ...
    let matches_root = match target_row.mode.as_str() {
        "local" => target_row
            .root_path
            .as_deref()
        ...
        "worktree" => {
            target_row
                ...
                .as_deref()
                .is_some_and(|path| path_matches(workspace_root, path))
                || parse_workspace_path(workspace_root).is_some_and(|(root_repo, root_dir)| {
                    root_repo == target_row.repo_name && root_dir == target_row.dir_name
                })
        }
        _ => false,
    };
    ...
    Some((
        target_row.remote.unwrap_or_else(|| "origin".into()),
        target_row.target?,
    ))
}
...
fn query_worktree_workspace_target_by_path(
    ...
    let root_path = workspace_root.to_string_lossy();
    query_worktree_workspace_target_by_stored_path(conn, root_path.as_ref()).or_else(|| {
        let canonical = workspace_root.canonicalize().ok()?;
        let canonical_path = canonical.to_string_lossy();
        if canonical_path == root_path {
            return None;
        }
        query_worktree_workspace_target_by_stored_path(conn, canonical_path.as_ref())
    })
}

fn query_worktree_workspace_target_by_stored_path(
    conn: &Connection,
    worktree_path: &str,
) -> Option<(String, String)> {
    let sql = format!(
        "SELECT r.remote, COALESCE(w.intended_target_branch, r.default_branch)
		 FROM workspaces w
		 JOIN repos r ON r.id = w.repository_id
		 WHERE w.worktree_path = ?1
		   AND COALESCE(w.mode, 'worktree') = 'worktree'
		   AND w.state {}",
        workspace_state::OPERATIONAL_FILTER,
    );
    let mut stmt = conn.prepare(&sql).ok()?;

    stmt.query_row(rusqlite::params![worktree_path], |row| {
        let remote: Option<String> = row.get(0)?;
        let target: Option<String> = row.get(1)?;
        Ok((remote, target))
    })
